...
import {
  Download, Rocket, RefreshCw, Edit2, Check, Palette, Sparkles, 
  Layout, Zap, FileText, Monitor, Terminal, Box, Type, Square, 
  Leaf, ArrowRight, Github, Mail, Trash2,MoreVertical,
} from "lucide-react";
...
const TEMPLATE_COMPONENTS: Record<Theme, React.LazyExoticComponent<any>> = {
  ...
  [Theme.Glassmorphism]: lazy(() => import("./components/templates/Glassmorphism")),
  [Theme.Cyberpunk]: lazy(() => import("./components/templates/Cyberpunk")),
  [Theme.ClassicSerif]: lazy(() => import("./components/templates/ClassicSerif")),
...
function App() {
  ...
  const handleReset = useCallback(() => {
    if (window.confirm("Are you sure you want to reset? You will lose your current progress.")) {
      setResumeData(null);
  ...
  >
    <div className="w-8 h-8 md:w-9 md:h-9 bg-indigo-600 rounded-lg md:rounded-xl flex items-center justify-center text-white shadow-lg transition-transform group-hover:rotate-6">
      <Zap size={18} fill="currentColor" />
    </div>
    <span className="font-bold text-base md:text-2xl tracking-tighter text-slate-900 whitespace-nowrap">
      Deploy<span className="xs:inline">MyResume</span>{" "}
      <span className="text-indigo-600 font-black">AI</span>
    </span>
  </div>
  ...
  {resumeData && (
    <div className="flex items-center gap-1.5 md:gap-3">
      ...
      >
        ...
        <span className="hidden lg:inline">{isEditing ? "Live Preview" : "Edit Content"}</span>
      </button>
            ...
            {Object.values(Theme).map((t) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-xs font-black whitespace-nowrap transition-all duration-300 border hover:scale-105 active:scale-95 ${
                  theme === t ? "bg-indigo-600 text-white border-indigo-600 shadow-md ring-4 ring-indigo-50" : "bg-white text-slate-500 hover:text-indigo-600 hover:border-indigo-200 border-slate-200 shadow-sm"
                }`}
              >
        ...
        {!resumeData ? (
          ...
          <div className="p-4 md:p-8 lg:p-12 bg-slate-100/50 min-h-screen">
            <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-2xl overflow-hidden border border-slate-200 transition-all animate-in fade-in zoom-in duration-700">
              <Suspense fallback={<div className="h-96 flex items-center justify-center font-bold text-slate-400">Loading Template...</div>}>
                <TemplateRenderer
                  theme={theme}
                  data={resumeData}
                  isEditing={isEditing}
 ...
 */
const LandingPage = React.memo(({ onUpload }: { onUpload: (data: ResumeData) => void }) => {
  return (
    <div className="flex flex-col items-center w-full">
      <section className="pt-20 pb-16 px-6 text-center max-w-5xl">
        ...
        <h1 className="font-black text-slate-900 mb-6 tracking-tight leading-[1.05] text-4xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl">
          <span className="block">
            Build your <span className="text-transparent bg-clip-text bg-linear-to-br from-indigo-600 via-purple-600 to-pink-500">Portfolio</span>
          </span>
          <span className="block text-slate-700 mt-2 sm:mt-3">in seconds.</span>
        </h1>
        <p className="text-lg md:text-2xl text-slate-500 mb-12 max-w-3xl mx-auto font-medium leading-relaxed italic">
          Turn a static PDF into a high-performance, responsive React portfolio. Choose your aesthetic and ship it.
        </p>
      ...
      <section className="w-full bg-slate-950 py-32 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-20">
            <h2 className="text-white text-3xl md:text-5xl font-black mb-4 tracking-tight">The Ship Cycle</h2>
            <div className="h-1.5 w-20 bg-indigo-500 mx-auto rounded-full"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8">
            <FunStep title="Upload" desc="Drop your PDF resume." icon={FileText} color="bg-blue-500" />
            <FunStep title="Analyze" desc="AI extracts your soul." icon={Zap} color="bg-yellow-400" />
            <FunStep title="Style" desc="Pick from 10+ themes." icon={Monitor} color="bg-purple-500" />
            <FunStep title="Tweak" desc="Edit content live." icon={Palette} color="bg-pink-500" />
            <FunStep title="Deploy" icon={Rocket} desc="Go live on GitHub." color="bg-emerald-500" />
          </div>
        </div>
      </section>
      ...
      <section className="w-full py-32 px-6 bg-white">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-6 tracking-tighter">Your Aesthetic, Your Choice.</h2>
          <p className="text-slate-500 mb-20 text-lg font-medium">Explore all 10 premium templates designed for impact.</p>
...
const FunStep = React.memo(({ num, title, desc, icon: Icon, color }: any) => (
  <div className="flex flex-col items-center group">
    <div className={`w-20 h-20 rounded-4xl ${color} flex items-center justify-center text-white mb-6 shadow-2xl group-hover:scale-110 transition-transform duration-500 group-hover:rotate-6`}>
      <Icon size={32} />
    </div>
    <div className="relative">
      <div className="text-4xl font-black text-white/10 absolute -top-4 -left-6">{num}</div>
      <h3 className="text-white text-xl font-bold mb-2 relative z-10">{title}</h3>
    </div>
    ...
    >
      ...
      >
        ...
        <div className={`absolute inset-0 transition-all duration-500 pointer-events-none ${
          sample.label === "Cyberpunk" ? "group-hover:bg-green-400/10" :
          sample.label === "Brutalist" ? "group-hover:shadow-[inset_0_0_0_4px_black]" :
  ...
  <footer className="w-full py-20 px-6 bg-slate-950 border-t border-white/5">
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
      <div className="text-center md:text-left">
        <p className="text-indigo-400 text-xs font-black uppercase tracking-[0.4em] mb-4">Architected By</p>
        <h4 className="text-4xl md:text-5xl font-black text-white tracking-tighter">Yash Batham</h4>
        <div className="flex items-center justify-center md:justify-start gap-6 mt-8">
          <a href="mailto:yashombatham@gmail.com" className="group flex items-center gap-3 text-slate-400 hover:text-white transition-all">
      ...
      <div className="flex flex-col items-center md:items-end text-slate-500 text-sm font-bold uppercase tracking-widest">
        <p className="text-white mb-2 tracking-normal capitalize font-black text-lg">
          DeployMyResume AI &copy; {new Date().getFullYear()}
        </p>
      </div>
    </div>
  </footer>
